...
/** Animated skeleton bar — shows while data is loading. */
...
export default function CompanyProfileEdit() {
  ...
  const [form, setForm] = useState({
    ...
    website: '', employee_count: '', location: '',
  });
  ...
  const inputCls =
    'w-full border border-outline-variant rounded-lg px-3 py-2 text-on-surface ' +
    'bg-surface-container-lowest focus:ring-1 focus:ring-primary outline-none ' +
    'transition-colors font-body-md disabled:opacity-50';
      ...
      <main className="flex-grow max-w-3xl w-full mx-auto px-gutter py-lg pb-24">
        ...
        <div className="bg-surface-container-lowest border border-surface-variant rounded-xl p-lg flex flex-col gap-md shadow-sm">
          ...
          <div className="flex items-center gap-md pb-md border-b border-surface-variant">
            <div className="w-24 h-24 rounded-xl border border-surface-variant bg-surface-container flex items-center justify-center overflow-hidden flex-shrink-0">
              {uploading ? (
                <Spinner className="text-primary" />
              ) : loading ? (
                <Skel className="w-full h-full rounded-xl" />
              ) : profile?.logo_url ? (
                <img src={getMediaUrl(profile.logo_url)} alt="Logo" className="w-full h-full object-cover" />
              ) : (
            ...
            <div>
              ...
              <button
                ...
                className="px-4 py-2 border border-primary text-primary rounded-lg font-label-md hover:bg-primary/5 transition-colors disabled:opacity-50"
              >
                Upload Logo
              </button>
            </div>
          ...
          <form onSubmit={handleSave} className="flex flex-col gap-md">
            ...
            <div className="flex flex-col gap-xs">
              ...
              {loading ? <Skel className="h-28" /> : (
                <textarea
                  ...
                  placeholder="Describe your company culture, mission, and what interns can expect..."
                />
              )}
            </div>
            ...
            <div className="flex justify-end pt-md">
              <button
                ...
                className="px-6 py-2 bg-primary text-on-primary rounded-lg flex items-center gap-2 hover:bg-surface-tint shadow-sm disabled:opacity-60 transition-opacity"
              >
